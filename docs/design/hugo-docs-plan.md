...
- **Content stays in `docs/`; no `content/` copy.** The main app's plan copied Markdown into
...
**Phase 4 — Serve at `/docs`.** Add the daemon route (mirrors the main app `frontend.py`): `GET /docs/{path}`
...
from the main app plan. If any of those are wanted later they're additive — none are load-bearing for
"serve `docs/` at `/docs`."
