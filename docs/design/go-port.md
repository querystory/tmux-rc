...
future in [cloud-architecture.md](../cloud-architecture/) gets built, because that world
...
1. **Operational liability** — the daemon runs unattended for days and Python's failure
   ...
   [deployment.md](../deployment/) + [durable-vertex-auth.md](../durable-vertex-auth/)
...
2. **Strategic liability** — the *future* is Go. The cloud relay
   ([cloud-architecture.md](../cloud-architecture/)) is specified as "a single Go Cloud Run
...
4. **Statefulness is a liability for an always-on daemon.** A persistent control-mode
   ...
   precisely the failure class [deployment.md](../deployment/) is trying to *remove*. The
...
of [durable-vertex-auth.md](../durable-vertex-auth/)'s recommendation (option A, the
...
- **The `.txt` prompts → `go:embed`.** `parser_prompt.txt` and `bootstrap_prompt.txt` are
  ...
  (which [deployment.md](../deployment/) notes bit the daemon — running from a worktree
...
- **Shared language with the relay — real and the main prize.** This is the one that
  matters. [cloud-architecture.md](../cloud-architecture/) specifies the relay as Go and
...
- **Single static binary for deployment — nice, modestly load-bearing.**
  [deployment.md](../deployment/) is moving to a systemd user unit, and its problem
...
- **Concurrency for many panes/sessions — marginal.** This is the weakest argument, so be
  ...
  cadence. A power user has, what, 14 sessions ([deployment.md](../deployment/)'s example)
