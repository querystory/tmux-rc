...
def test_row_counts_up_from_visible_last_line(monkeypatch):
    ...
    assert T.click("%1", expected_pid="1234", from_bottom=1, col=5)
    assert report(sent[0]) == "\x1b[<0;5;3M\x1b[<0;5;3m"


def test_no_mouse_mode_sends_nothing(monkeypatch):
    sent = fake_tmux(monkeypatch, "a\nb", sgr="0")
    assert not T.click("%1", expected_pid="1234", from_bottom=0, col=1)
    assert sent == []


def test_tap_on_history_sends_nothing(monkeypatch):
    sent = fake_tmux(monkeypatch, "a\nb")
    assert not T.click("%1", expected_pid="1234", from_bottom=2, col=1)
    assert sent == []
...
def test_out_of_bounds_coordinates_send_nothing(monkeypatch):
    sent = fake_tmux(monkeypatch, "a\nb")
    assert not T.click("%1", expected_pid="1234", from_bottom=-1, col=1)
    assert not T.click("%1", expected_pid="1234", from_bottom=0, col=0)
    assert not T.click("%1", expected_pid="1234", from_bottom=0, col=81)
    assert sent == []
...
def test_recycled_pane_never_receives_a_stale_click(monkeypatch):
    ...
    with pytest.raises(T.PaneChangedError):
        T.click("%1", from_bottom=0, col=1, expected_pid="1234")
    assert sent == []
...
def test_click_endpoint_canonicalizes_and_reparses_only_sent_clicks(monkeypatch, sent):
    ...
    response = TestClient(app).post("/api/panes/work:0.0/click", json={"from_bottom": 2, "col": 4})
    assert response.status_code == 200
    assert response.json() == {"sent": sent}
    click.assert_called_once_with("%7", 2, 4, expected_pid="42")
