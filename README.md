...
| `TMUXRC_TARGET` | unset (all panes) | restrict watching to one pane. Always reliable: a pane id (`%3`) or a numeric tmux address (`session:window_index[.pane_index]`, e.g. `work:0.0` — indices, not the window name). Also accepted: the pane's derived label (`name` / `name.N`) — which is the window name only when you named the window, and otherwise the *already-qualified* `session:window_index` form above, so a bare session or cwd name never matches. The card title in the UI is not a valid target either; use a pane id when in doubt |
