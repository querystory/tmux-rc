...
| `TMUXRC_TARGET` | unset (all panes) | restrict watching to one pane. Always reliable: a pane id (`%3`) or a numeric tmux address (`session:window_index[.pane_index]`, e.g. `work:0.0` — indices, not the window name). Also accepted: the pane's derived label (`name` / `name.N`) — a window name that doesn't look like a tmux default, else `<base>:<window_index>` where base is the session name, or the cwd basename when the session name looks like a default too. Either way it is qualified, so a bare session or cwd name never matches. The card title in the UI is not a valid target either; use a pane id when in doubt |
