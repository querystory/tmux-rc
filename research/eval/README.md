...
| `17_claude_resume_cursor_picker` | **#206**: /resume cursor list with no digits → question, answer_style=cursor |
| `18_claude_done_marker_with_draft` | **#210**: finished turn + unsent draft in the box → idle, not running |
| `19_codex_orchestrating_claude` | **#208**: Codex driving a Claude pane via tmux → captured Claude chrome is content; the pane's own `gpt-…` bar settles tool=codex |
| `20_codex_working_chrome` | ordinary Codex tool/progress chrome stays Codex while discussing Claude |
| `21_codex_idle_custom_footer` | Codex with a custom footer and no model name stays Codex and idle |
