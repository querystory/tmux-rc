...
Usage: TMUXRC_LIVE_MODELS='[…]' smoke_live_ws.py <label>
(keys from ~/.config/tmux-rc/openai.env)"""
...
class Watcher:
    ...
    snapshots = {  # noqa: RUF012 - one fake watcher, one instance; nothing shares it
        "%1": [{"id": "s1", "text": "Run the tests? 1) yes 2) no", "ts": 1.0}],
        "%2": [{"id": "s2", "text": "$ ", "ts": 1.0}],
    }
    v = 1
        ...
        return [
            {
                ...
                "activity": "waiting",
                "tmux_active": True,
                "headline": "asking whether to run tests",
                "summary": None,
                "question": "Run the tests? 1) yes 2) no",
                "history": [],
            },
            {
                "pane_id": "%2",
    ...
    async def wait_for_state_change(self, since, timeout):  # noqa: ASYNC109 - the real Watcher's signature
        if self.v == 1:
...
def tts_pcm16k(text: str) -> bytes:
    ...
    req = urllib.request.Request(
        ...
        data=json.dumps(
            {"model": "gpt-4o-mini-tts", "input": text, "voice": "alloy", "response_format": "pcm"}
        ).encode(),
    )
    pcm24 = urllib.request.urlopen(req, timeout=60).read()
    x = struct.unpack(f"<{len(pcm24) // 2}h", pcm24[: len(pcm24) & ~1])
    n = len(x)
    out = []
...
def main():
    ...
    frames = [
        speech[i : i + 8192] for i in range(0, len(speech), 8192)
    ]  # 4096 samples, like the browser
    log = []
    t0 = time.monotonic()
    ...
    with c.websocket_connect(f"/api/live-mode?model={LABEL}") as ws:
        ...
        def pump(until, budget):
            while time.monotonic() - t0 < budget:
                m = ws.receive_json()
                log.append((round(time.monotonic() - t0, 2), m))
                if until(m):
                    return m
            return None
        ...
        for f in frames:
            ws.send_json({"action": "audio", "data": base64.b64encode(f).decode()})
            time.sleep(0.05)
        t_speech = time.monotonic() - t0
        pump(lambda m: m["type"] == "typed", 45)
        pump(
            lambda m: m["type"] == "turn_complete" and any(x[1]["type"] == "typed" for x in log), 25
        )
        ws.send_json({"action": "stop"})
        try:
            while True:
                log.append((round(time.monotonic() - t0, 2), ws.receive_json()))
        except Exception:  # noqa: BLE001 - a smoke run reports, it does not propagate
            pass

    kinds = [m["type"] if m["type"] != "status" else "status:" + m["status"] for _, m in log]
    pre_speech = [
        k
        for (t, m), k in zip(log, kinds, strict=True)
        if t < t_speech and k not in ("status:connecting", "status:listening")
    ]
    ...
    print(
        f"meter final: turns={emitted[-1]['turns'] if emitted else None} "
        f"in={folded.get('in_tokens')} out={folded.get('out_tokens')} "
        f"cost=${folded.get('cost', 0):.4f} "
        f"model={emitted[-1]['model'] if emitted else None}"
    )
    ok = (
        bool(typed)
        and not pre_speech
        and folded.get("cost", 0) > 0
