    ...
    try:
        async with client.aio.live.connect(model=model, config=cfg) as s:
            ...
            await s.send_client_content(
                turns=types.Content(
                    role="user", parts=[types.Part(text="Say hello in five words.")]
                ),
                turn_complete=True,
            )
            async for msg in s.receive():
                ...
                if sc.turn_complete:
                    break
            print(
                f"OK   {model:55s} {region:12s} connect={t_conn:.2f}s "
                f"ttfa={first and round(first, 2)}s turn={time.monotonic() - t1:.2f}s "
                f"audio={nbytes // 1024}KB said={text.strip()!r}"
            )
    except Exception as e:  # noqa: BLE001 - a probe reports every failure, never raises
...
async def main():
    for m in CANDIDATES:
        for region in ("us-central1",):
            try:
                await asyncio.wait_for(probe(m, region), timeout=25)
            except TimeoutError:
                print(f"HANG {m:55s} {region:12s} no reply in 25s")
