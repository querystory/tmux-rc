...
AZURE_OPENAI_ENDPOINT (either *.openai.azure.com or *.cognitiveservices.azure.com;
...
from harness_gemini import (
    CASES,
    SNAPSHOT,
)

from openbus.live_providers import (
    LiveModel,
    openai_endpoint,
)
...
async def run_case(utt, pane, sub, expect_tool):
    async with websockets.connect(URL, additional_headers=HDR, max_size=None) as ws:
        ...
        while time.monotonic() - t0 < 20:
            ev = json.loads(await asyncio.wait_for(ws.recv(), 20))
            t = ev.get("type", "")
            if t == "response.function_call_arguments.done":
                call = json.loads(ev["arguments"])
                t_call = time.monotonic() - t0
            elif t == "response.output_audio_transcript.done":
                said += ev.get("transcript", "")
            elif t == "error":
                print("ERR", ev.get("error"))
                break
    ...
    ok = (
        (
            call is not None
            and call.get("pane_id") == pane
            and sub.lower() in str(call.get("keys", "")).lower()
        )
        if expect_tool
        else call is None
    )
    print(
        ...
        f"{utt[:52]:52s} -> {json.dumps(call)}  said={said.strip()[:60]!r}"
    )
    return ok
